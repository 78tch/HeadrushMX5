# HeadrushMX5
## 0.なによりもまず
MX5を、自宅の壁コンセントに2ピンのみつないで、ヘッドホンで使用したところ、ノイズがひどかった。
そういう場合、アースも接続するか、モバイルバッテリーから電源をとると、ノイズを低減できることがある。
※別の電気機器とアースを共有していると、却ってノイズをもらうこともありうる？要検証。
MX5 の電源仕様は、「12V、3.0A、センタープラスの外径5.5mm、内径2.1mm」である。
また、付属のACアダプタの電源ケーブルは、「2P + アースリード」である。
アース線をネジ留めできる壁コンセントというのは、洗濯機や冷蔵庫やエアコン用ぐらいで、普通の居室の壁コンセントには、アースがついていないこともある。
私の自宅では、付属ACアダプターでアースをつながなかった場合、弦から両手を離した無音時に、「ジー」というノイズが出てしまった。
...
- 残課題：自宅ではヘッドホン使用が基本なので、ヘッドホン使用時において「ACアダプターをアースにつなぐ」のと、「モバイルバッテリーから電源をとる」のとで、どちらがよりノイズが少ないのか、比較したい。
...
### 1-2. 演奏時は「Hybridモード」が推奨
初期のファームウェアでは、「シーン」が「ON/OFF」を繰り返すことができなかったため、フットスイッチには「シーン」ではなく「モデル」の「ON/OFF」を割り当てるしかなかった。
しかしファームウェアがアップデートされて、「シーン」でも繰り返し「ON/OFF」できるようになったため、「Hybridモード」の使い勝手が向上した。
...
### 1-2. FS1だけでなく、FS2やFS3もSTOMPのON/OFFとして使うならSTOMPモードが必須
